...
<body>
      ...
      <ul class="nav-links">
       ...
       <li >
          <a href="#" onclick="window.location.href='<%= request.getContextPath() %>/view/StockLogin.jsp'">
            ...
            <span class="links_name">Suppliers Details</span>
          </a>
        </li>
      </ul>
  </div>
 ...
 <%
 int i=1;
 List<Supplier>lstSupplier = (List<Supplier>) request.getAttribute("lstSuppliers");
 %>
 
 <%
 if(lstSupplier!=null){
   for(Supplier cust : lstSupplier){
     %>
     <tr>
    
     <td><%= cust.getS_Name() %></td>
     <td><%= cust.getS_Address() %></td>
     <td><%= cust.getS_Phone() %></td>
     </tr>
     <%
   }
 }else{
