...
<%@page import="controller.ViewOrderController" %>
<!DOCTYPE html>
...
<body>
      ...
      <ul class="nav-links">
       ...
       <li >
          <a href="#" onclick="window.location.href='<%= request.getContextPath() %>/view/StockLogin.jsp'">
            ...
            <span class="links_name"> Product Orders</span>
          </a>
        </li>
      </ul>
  </div>
 ...
 <%
 int i=1;
 List<Orders>lstCustomer = (List<Orders>) request.getAttribute("lstOrders");
 %>
 
 <%
 if(lstCustomer!=null){
   for(Orders cust : lstCustomer){
     %>
     <tr>
     <td><%= cust.getO_ID() %></td>
     <td><%= cust.getO_ProductName() %></td>
     <td><%= cust.getO_ProductQTY() %></td>
     <td><%= cust.getC_ID() %></td>
     <td><%= cust.getO_Date() %></td>
