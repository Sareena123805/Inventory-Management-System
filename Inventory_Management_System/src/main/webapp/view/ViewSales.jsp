...
<%@page import="controller.ViewSalesController" %>
<!DOCTYPE html>
...
<body>
 ...
 <%
 int i=1;
 List<Sales>lstCustomer = (List<Sales>) request.getAttribute("lstSales");
 %>
 
 <%
 if(lstCustomer!=null){
   for(Sales cust : lstCustomer){
     %>
     <tr>
     <td><%= cust.getSl_ID() %></td>
     <td><%= cust.getSl_ProductName() %></td>
     <td><%= cust.getSl_ProductQTY() %></td>
     <td><%= cust.getSl_Price() %></td>
     <td><%= cust.getS_Date() %></td>
     </tr>
     <%
   }
 }else{
